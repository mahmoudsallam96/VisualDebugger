...
public class TpsRouteDebuggingVisualizer extends DebuggingInfoVisualizerBase {
  ...
  public void doVisualizationFurther(ObjectDiagram diagram) {
    final var plantUMLString = TpsRouteDebuggingVisualizer.toPlantUMLString(diagram);
    SharedState.setLastPlantUMLDiagram(plantUMLString);
    try {
      final byte[] pngData = TpsRouteDebuggingVisualizer.toImage(plantUMLString, FileFormat.PNG);
  ...
  static String toPlantUMLString(final ObjectDiagram objectDiagram) {
    ...
    TpsRouteDebuggingVisualizer.addObjectsToDiagramAndCollectLinks(
        stringBuilder, sortedObjects, mapLinks);

    // Add links.
    TpsRouteDebuggingVisualizer.addLinksToDiagram(
    ...
    if (!objectDiagram.getPrimitiveRootValues().isEmpty()) {
      TpsRouteDebuggingVisualizer.addPrimitiveRootValuesToDiagram(objectDiagram, stringBuilder);
    }
  ...
  private static void addObjectsToDiagramAndCollectLinks(
    ...
    for (final ODObject object : sortedObjects) {
      ...
      if (TpsRouteDebuggingVisualizer.isPrimitiveJavaMap(object)) {
        TpsRouteDebuggingVisualizer.doPrimitiveMapVisualisation(
      ...
      stringBuilder.append(
          String.format(
              ...
              TpsRouteDebuggingVisualizer.shortenTypeName(object.getType()),
              object.hashCode()));
  ...
  private static boolean isPrimitiveJavaMap(final ODObject object) {
    return TpsRouteDebuggingVisualizer.isMap(object)
        && TpsRouteDebuggingVisualizer.isPrimitive(object);
  }
